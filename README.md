...
Download the data from a public S3 bucket. 
...
### Step 4: src/upload_data.py
Write raw data to your S3 bucket. 
...
- Run the following command: docker run --env-file=config/.mysqlconfig --mount type=bind,source="$(pwd)"/data,target=/app/data insurance python3 src/sql/model.py --RDS True 
